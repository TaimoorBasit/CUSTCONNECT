...
app.get('/api/ping', (req, res) => {
  res.json({
    success: true,
    message: 'Backend server is running - v4-final-fix',
    timestamp: new Date().toISOString()
  });
});
  ...
  try {
    ...
    const existing = await prisma.user.findFirst({
      where: {
        OR: [
          { email: email },
          { email: 'admin' } // check legacy username too
        ]
      }
    });

    if (existing) {
      // Force update password to 'password'
      ...
      return res.json({ message: 'User updated. Login with: admin@admin.com / password' });
    }
